...
export const updateBlog = createAsyncThunk('blogs/updateBlog', async (blog: any) => {
...
const blogsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(createBlog.fulfilled, (state: any, action: any) => {
                state.data.push(action.payload);
            })
            .addCase(updateBlog.fulfilled, (state: any, action) => {
                const index = state.data.findIndex((blog: any) => blog.id === action.payload.id);
                if (index !== -1) {
                    state.data[index] = action.payload;
                }
            })
            .addCase(deleteBlog.fulfilled, (state, action) => {
                state.data = state.data.filter((blog: any) => blog.id !== action.payload);
            });
    },
});
